...
-export([identity/0, init/1, lookat/3, ortho/2, ortho/6, perspective/3, perspective/4,
         pick/5, frustum/6, matrix/1, inv_matrix/1, inverse/1, translate/2, rotate/2, rotate/3,
         scale/2, mul/1, mul/2, project/4, unproject/4]).%% Initializes matrices
...
mul([#e3d_transf{} = A, #e3d_transf{} = B | T]) ->
  mul([mul(B, A) | T]);
mul([#e3d_transf{} = A]) ->
  A.
...
unproject({WinX, WinY, WinZ},
          #e3d_transf{inv = InvMV},
          #e3d_transf{inv = InvPr},
          {V0, V1, V2, V3}) ->
...
lookat(Pos, Look, Up) ->
  ...
  Right =
    e3d_vec:norm(
      e3d_vec:cross(Dir, e3d_vec:norm(Up))),
  NewUp =
    e3d_vec:norm(
      e3d_vec:cross(Right, Dir)),
  AsList =
    [tuple_to_list(Right),
     0.0,
     tuple_to_list(NewUp),
     0.0,
     tuple_to_list(e3d_vec:neg(Dir)),
     0.0,
     0.0,
     0.0,
     0.0,
...
ortho(Left, Right, Bottom, Top, Near, Far) ->
  ...
  Mat0 = {2.0, O, O, O, 2.0, O, O, O, -2.0, O, O, O},
...
%% @end
%%--------------------------------------------------------------------
-spec perspective(Fov :: float(), Near :: float(), Far :: float()) -> transform().
...
frustum(Left, Right, Bottom, Top, Near, Far) ->
  ...
  Persp = {E, O, O, O, O, F, O, O, A, B, C, -I, O, O, D, O},
  InvPersp = e3d_mat:invert(Persp),
...
pick(X, Y, W, H, {X0, Y0, X1, Y1}) ->
  ...
  I = 1.0,
  O = 0.0,
  Pick = {Sx, O, O, O, O, Sy, O, O, O, O, I, O, Tx, Ty, O, I},
  init(Pick).
